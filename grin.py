...
def colorize(s, fg=None, bg=None, bold=False, underline=False, reverse=False):
    ...
    styles = (str(style).encode("ascii") for style in style_fragments)
    style_start = b"\x1b[" + b";".join(styles) + b"m"
    style_end = b"\x1b[0m"
    ...
    def report(self, context_lines, filename=None):
        ...
        if not self.options.show_match:
            ...
            if self.options.show_emacs:
                template = b"%(filename)s:%(lineno)s: %(line)s"
            elif self.options.show_line_numbers:
                template = b"%(lineno)5s %(sep)s %(line)s"
            else:
                template = b"%(line)s"
...
class FileRecognizer(object):
        ...
        with open(filename, "rb") as f:
            ...
            if marker == GZIP_MAGIC:
                with gzip.open(filename) as fp:
                    try:
                        return not self._is_binary_file(fp)
                    except IOError:
    ...
    def recognize_directory(self, filename):
        ...
        if (
            ...
            and basename.startswith(b".")
            and basename not in (b".", b"..")
        ):
            return "skip"
    ...
    def recognize_file(self, filename):
        ...
        """
        basename = os.path.split(filename)[-1]
        if self.skip_hidden_files and basename.startswith(b"."):
            return "skip"
        if self.skip_backup_files and basename.endswith(b"~"):
            return "skip"
        ...
        if ext in self.skip_exts_simple or ext.startswith(b".~"):
            return "skip"
